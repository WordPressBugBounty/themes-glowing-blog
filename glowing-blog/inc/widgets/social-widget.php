<?php
if ( ! class_exists( 'Glowing_Blog_Social_Widget' ) ) {
	/**
	 * Adds Glowing Blog Social Widget.
	 */
	class Glowing_Blog_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$glowing_blog_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'glowing-blog' ),
			);
			parent::__construct(
				'glowing_blog_social_widget',
				__( 'Adore Widget: Social Widget', 'glowing-blog' ),
				$glowing_blog_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'social_icon_label' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Icon Label %d :', 'glowing-blog' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'glowing-blog' ), $i ); ?></label>
